...
Stops and moves by definition are different and heterogeneous trajectory elements. A stop may have a spatial position, a start and end time, a category, and a set of attributes related to the category. \hl{For example, a stop at a hotel may have the attributes spatial location of the hotel, the start and end time the moving object stayed at the hotel, the number of stars, rate, price of the hotel}, etc. A move starts and ends in a stop and may be characterized by different attributes as the average speed, traveled distance, sequence of followed streets, duration, the sequence of raw points, etc. These attributes are defined according to the needs of the application. \hl{From these examples we notice that stops and moves are characterized by different attributes, and they must be treated as different trajectory elements.}
...
In this section we present a running example, comparing SMSM and MSM, since SMSM is and extension of MSM.
